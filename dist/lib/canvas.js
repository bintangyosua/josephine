...
exports.convertToRankingData = convertToRankingData;
exports.generateRankingImageBuffer = generateRankingImageBuffer;
...
async function generateRankingImageBuffer(data) {
    const itemHeight = 40;
    const padding = 5;
    ...
    try {
        ...
        const possiblePaths = [
            path_1.default.join(__dirname, "../../images/chat.png"),
            path_1.default.join(__dirname, "../images/chat.png"),
            path_1.default.join(__dirname, "./images/chat.png"),
            path_1.default.join(process.cwd(), "images/chat.png"),
            path_1.default.join(process.cwd(), "src/images/chat.png"),
        ];
        // Try each path until we find the image
        for (const imagePath of possiblePaths) {
            if (fs_1.default.existsSync(imagePath)) {
                chatIcon = await (0, canvas_1.loadImage)(imagePath);
        ...
        if (!chatIcon) {
            console.warn("Chat icon not found in any of the expected locations. Creating a fallback icon.");
            const iconCanvas = (0, canvas_1.createCanvas)(16, 16);
            ...
            chatIcon = await (0, canvas_1.loadImage)(dataURL);
        }
    }
    catch (error) {
    ...
    data.forEach((item, index) => {
        ...
        if (item.rank === 2)
            rankColor = "#A9A9A9"; // Silver for #2
        else if (item.rank === 3)
            rankColor = "#CD7F32"; // Bronze for #3
        else if (item.rank > 3)
