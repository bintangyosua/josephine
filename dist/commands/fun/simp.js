...
const Simp = {
    ...
    async execute(interaction) {
        ...
        let comment = "";
        if (simpRate < 30)
            comment = "Masih bisa diselamatkan 😌";
        else if (simpRate < 60)
            comment = "Udah mulai bahaya bro 😳";
        else if (simpRate < 85)
            comment = "Down bad alert 🚨";
        else
            comment = "Lo udah terlalu dalam bro 😭";
        await interaction.reply(`💘 **${author.username}** simping to **${target.username}** at **${simpRate}%** level.\n_${comment}_`);
    },
};
